...
namespace Unity.Entities
{
    ...
    public static unsafe partial class TypeManager
    {
        ...
        public static void CollectComponentTypes(IEnumerable<Assembly> assemblies, HashSet<Type> componentTypeSet)
        {
            foreach (var assembly in assemblies)
            {
                IsAssemblyReferencingEntitiesOrUnityEngine(assembly, out var isAssemblyReferencingEntities,
        ...
        public static void EarlyInitAssembly(Assembly ass)
        {
            foreach (var type in ass.GetTypes())
            {
                if (type.Name.StartsWith("__JobReflectionRegistrationOutput__"))
                {
                    Debug.Log($"InvokeJobReflectionRegistrationOutput. assembly:{ass.GetName().Name}  type:{type.FullName}");
