...
namespace SGame.UI{
	...
	public partial class UIPet : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIPet));

		private UI_PetUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_PetUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
