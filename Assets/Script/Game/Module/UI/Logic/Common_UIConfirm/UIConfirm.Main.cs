...
namespace SGame.UI{
	...
	public partial class UIConfirm : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIConfirm));

		private UI_ConfirmUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_ConfirmUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
