...
namespace SGame.UI{
	...
	public partial class UIFriendDetail : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIFriendDetail));

		private UI_FriendDetailUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_FriendDetailUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
