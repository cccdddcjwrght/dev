...
namespace SGame.UI{
	...
	public partial class UISuit : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UISuit));

		private UI_SuitUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_SuitUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
