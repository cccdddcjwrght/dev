...
namespace SGame.UI{
	...
	public partial class UIHotFood : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIHotFood));

		private UI_HotFoodUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_HotFoodUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
