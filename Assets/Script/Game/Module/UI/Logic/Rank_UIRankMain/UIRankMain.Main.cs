...
namespace SGame.UI{
	...
	public partial class UIRankMain : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIRankMain));

		private UI_RankMainUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_RankMainUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
