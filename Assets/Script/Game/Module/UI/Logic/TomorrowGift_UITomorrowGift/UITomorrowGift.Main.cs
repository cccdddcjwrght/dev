...
namespace SGame.UI{
	...
	public partial class UITomorrowGift : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UITomorrowGift));

		private UI_TomorrowGiftUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_TomorrowGiftUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
