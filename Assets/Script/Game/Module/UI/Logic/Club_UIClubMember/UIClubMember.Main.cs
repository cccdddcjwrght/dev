...
namespace SGame.UI{
	...
	public partial class UIClubMember : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIClubMember));

		private UI_ClubMemberUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_ClubMemberUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
