...
namespace SGame.UI{
	...
	public partial class UIUseHelp : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIUseHelp));

		private UI_UseHelpUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_UseHelpUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
