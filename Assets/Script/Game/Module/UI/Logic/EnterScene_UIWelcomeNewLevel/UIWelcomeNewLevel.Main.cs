...
namespace SGame.UI{
	...
	public partial class UIWelcomeNewLevel : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIWelcomeNewLevel));

		private UI_WelcomeNewLevelUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_WelcomeNewLevelUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
