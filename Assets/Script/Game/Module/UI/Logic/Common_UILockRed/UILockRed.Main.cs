...
namespace SGame.UI{
	...
	public partial class UILockRed : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UILockRed));

		private UI_LockRedUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_LockRedUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
