...
namespace SGame.UI{
	...
	public partial class UIClubTask : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIClubTask));

		private UI_ClubTaskUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_ClubTaskUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
