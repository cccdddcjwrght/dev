...
namespace SGame.UI{
	...
	public partial class UIClubReward : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIClubReward));

		private UI_ClubRewardUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_ClubRewardUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
