...
namespace SGame.UI{
	...
	public partial class UIFinger : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIFinger));

		private UI_FingerUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_FingerUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
