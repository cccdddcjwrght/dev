...
namespace SGame.UI{
	...
	public partial class UIWorkerCollectProperty : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIWorkerCollectProperty));

		private UI_WorkerCollectPropertyUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_WorkerCollectPropertyUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
