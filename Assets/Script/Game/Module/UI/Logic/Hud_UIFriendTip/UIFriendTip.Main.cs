...
namespace SGame.UI{
	...
	public partial class UIFriendTip : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIFriendTip));

		private UI_FriendTipUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_FriendTipUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
