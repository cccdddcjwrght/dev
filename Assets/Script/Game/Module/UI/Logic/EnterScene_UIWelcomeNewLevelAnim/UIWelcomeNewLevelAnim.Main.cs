...
namespace SGame.UI{
	...
	public partial class UIWelcomeNewLevelAnim : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIWelcomeNewLevelAnim));

		private UI_WelcomeNewLevelAnimUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_WelcomeNewLevelAnimUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
