...
namespace SGame.UI{
	...
	public partial class UIClubSelect : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIClubSelect));

		private UI_ClubSelectUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_ClubSelectUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
