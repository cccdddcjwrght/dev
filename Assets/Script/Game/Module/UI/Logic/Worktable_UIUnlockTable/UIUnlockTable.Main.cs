...
namespace SGame.UI{
	...
	public partial class UIUnlockTable : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIUnlockTable));

		private UI_UnlockTableUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_UnlockTableUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
