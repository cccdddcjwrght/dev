...
namespace SGame.UI{
	...
	public partial class UILockPanel : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UILockPanel));

		private UI_LockPanelUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_LockPanelUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
