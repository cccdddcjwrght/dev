...
namespace SGame.UI{
	...
	public partial class UICookbook : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UICookbook));

		private UI_CookbookUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onOpen += OnShow;
			m_view = context.content as UI_CookbookUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
