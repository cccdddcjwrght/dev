...
namespace SGame.UI{
	...
	public partial class UIBuffShop : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIBuffShop));

		private UI_BuffShopUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_BuffShopUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
