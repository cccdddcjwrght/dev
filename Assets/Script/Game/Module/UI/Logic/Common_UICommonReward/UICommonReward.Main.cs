...
namespace SGame.UI{
	...
	public partial class UICommonReward : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UICommonReward));

		private UI_CommonRewardUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_CommonRewardUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
