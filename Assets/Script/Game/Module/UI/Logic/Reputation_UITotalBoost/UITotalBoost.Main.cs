...
namespace SGame.UI{
	...
	public partial class UITotalBoost : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UITotalBoost));

		private UI_TotalBoostUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_TotalBoostUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
