...
namespace SGame.UI{
	...
	public partial class UIFlight : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIFlight));

		private UI_FlightUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_FlightUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
