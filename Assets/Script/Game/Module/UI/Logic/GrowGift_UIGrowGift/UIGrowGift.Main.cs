...
namespace SGame.UI{
	...
	public partial class UIGrowGift : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIGrowGift));

		private UI_GrowGiftUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_GrowGiftUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
