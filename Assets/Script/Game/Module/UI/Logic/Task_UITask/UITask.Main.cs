...
namespace SGame.UI{
	...
	public partial class UITask : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UITask));

		private UI_TaskUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_TaskUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
