...
namespace SGame.UI{
	...
	public partial class UIPropertyInfo : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIPropertyInfo));

		private UI_PropertyInfoUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_PropertyInfoUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
