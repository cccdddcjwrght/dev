...
namespace SGame.UI{
	...
	public partial class UICustomerbookUp : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UICustomerbookUp));

		private UI_CustomerbookUpUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_CustomerbookUpUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
