...
namespace SGame.UI{
	...
	public partial class UITempShop : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UITempShop));

		private UI_TempShopUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_TempShopUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
