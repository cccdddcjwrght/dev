...
namespace SGame.UI{
	...
	public partial class UIGuideDialogue : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIGuideDialogue));

		private UI_GuideDialogueUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_GuideDialogueUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
