...
namespace SGame.UI{
	...
	public partial class UIRoomExclusive : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIRoomExclusive));

		private UI_RoomExclusiveUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_RoomExclusiveUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
