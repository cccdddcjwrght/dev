...
namespace SGame.UI{
	...
	public partial class UIFriend : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIFriend));

		private UI_FriendUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_FriendUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
