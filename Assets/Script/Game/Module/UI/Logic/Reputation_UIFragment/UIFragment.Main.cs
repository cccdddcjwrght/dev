...
namespace SGame.UI{
	...
	public partial class UIFragment : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIFragment));

		private UI_FragmentUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_FragmentUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
