...
namespace SGame.UI{
	...
	public partial class UIGuideBack : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIGuideBack));

		private UI_GuideBackUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_GuideBackUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
