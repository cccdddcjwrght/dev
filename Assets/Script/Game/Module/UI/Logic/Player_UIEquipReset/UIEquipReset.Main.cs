...
namespace SGame.UI{
	...
	public partial class UIEquipReset : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIEquipReset));

		private UI_EquipResetUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_EquipResetUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
