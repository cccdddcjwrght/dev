...
namespace SGame.UI{
	...
	public partial class UIEquipPreview : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIEquipPreview));

		private UI_EquipPreviewUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_EquipPreviewUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
