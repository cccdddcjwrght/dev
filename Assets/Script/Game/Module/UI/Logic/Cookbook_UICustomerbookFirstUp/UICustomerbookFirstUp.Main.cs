...
namespace SGame.UI{
	...
	public partial class UICustomerbookFirstUp : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UICustomerbookFirstUp));

		private UI_CustomerbookFirstUpUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_CustomerbookFirstUpUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
