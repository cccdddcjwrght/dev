...
namespace SGame.UI{
	...
	public partial class UIPetBorn : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIPetBorn));

		private UI_PetBornUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_PetBornUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
