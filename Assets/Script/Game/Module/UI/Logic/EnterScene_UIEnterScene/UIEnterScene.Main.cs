...
namespace SGame.UI{
	...
	public partial class UIEnterScene : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIEnterScene));

		private UI_EnterSceneUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_EnterSceneUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
