...
namespace SGame.UI{
	...
	public partial class UIEnterSceneTemp : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIEnterSceneTemp));

		private UI_EnterSceneTempUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_EnterSceneTempUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
