...
namespace SGame.UI{
	...
	public partial class UIGetWorkerTips : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIGetWorkerTips));

		private UI_GetWorkerTipsUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_GetWorkerTipsUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
