...
namespace SGame.UI{
	...
	public partial class UIPetTips : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIPetTips));

		private UI_PetTipsUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_PetTipsUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
