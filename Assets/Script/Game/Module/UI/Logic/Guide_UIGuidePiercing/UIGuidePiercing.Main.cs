...
namespace SGame.UI{
	...
	public partial class UIGuidePiercing : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIGuidePiercing));

		private UI_GuidePiercingUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_GuidePiercingUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
