...
namespace SGame.UI{
	...
	public partial class UIExchangeTask : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIExchangeTask));

		private UI_ExchangeTaskUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_ExchangeTaskUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
