...
namespace SGame.UI{
	...
	public partial class UILevelCompleted : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UILevelCompleted));

		private UI_LevelCompletedUI m_view;

		public void OnInit(UIContext context)
		{
			context.onClose += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_LevelCompletedUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
