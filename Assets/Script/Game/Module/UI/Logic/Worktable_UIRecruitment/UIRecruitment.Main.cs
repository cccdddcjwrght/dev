...
namespace SGame.UI{
	...
	public partial class UIRecruitment : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIRecruitment));

		private UI_RecruitmentUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_RecruitmentUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
