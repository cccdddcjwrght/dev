...
namespace SGame.UI{
	...
	public partial class UIWorkerFirstGet : IUIScript
	{
		private static ILog log = LogManager.GetLogger("ui." + nameof(UIWorkerFirstGet));

		private UI_WorkerFirstGetUI m_view;

		public void OnInit(UIContext context)
		{
			context.onUninit += OnClose;
			context.onShown += OnShow;
			m_view = context.content as UI_WorkerFirstGetUI;
			BeforeInit(context);
			InitUI(context);
			InitEvent(context);
			InitLogic(context);
