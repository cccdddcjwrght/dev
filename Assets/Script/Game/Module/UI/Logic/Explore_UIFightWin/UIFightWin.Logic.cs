...
namespace SGame.UI{
	...
	public partial class UIFightWin
	{
		...
		partial void InitLogic(UIContext context){
			...
			if (config.IsValid()) 
			{
				awardList = DataCenter.BattleLevelUtil.GetReward(config.GetRewardId1Array(), config.GetRewardNum1Array());
				adAwardList = DataCenter.BattleLevelUtil.GetReward(config.GetRewardId2Array(), config.GetRewardNum2Array());
